...
l <- lm(data = d, Final_plantvol ~ Vole_Presence)
summary(l)
...
g1 <- ggplot(a, aes(x=Plant_cover2, y=Bird_visit6)) +
  geom_col(fill="blue") + 
  xlab("A. splendens cover (%)") + ylab("No. of visits by predatory birds/plot")

g1
g2 <- ggplot(b, aes(x=Plant_cover2, y=Feeding_freq)) +
  geom_col(fill="blue") +
  xlab("A. splendens cover (%)") + ylab("Feeding frequency (%)")
g2
g3 <- ggplot(c, aes(x=Plant_cover2, y=Vole_mortality)) +
  geom_col(fill="blue") +
xlab("A. splendens cover (%)") + ylab("Mortality rate (%)")
g3
chisq.test( y=Bird_visit6, x=Plant_cover2)
```
