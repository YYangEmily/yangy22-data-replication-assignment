...
```
I summarized change in plant volume according to vole presence and made a barplot. The error bars are mean +/- the standard error. I was unable to make one sided error bars and have them both below and above the mean. I used geom_signif to add in significance stars but it broke down Setup Params Can only handle data with groups that are plotted on the x-axis. 
...
n <- lm(data = d, Birds_total15 ~ Plant_cover)
summary(n)
...
one.aov <- aov(Birds_total15 ~ Plant_cover, data = d)
summary(one.aov)
...
g <- ggplot(q) +
  geom_col(aes(y=mean, x=Bird_visits), fill="forestgreen") +
  geom_errorbar( aes(x=Bird_visits,ymin=mean-se, ymax=mean+se), width=0.2, colour="black") +
ylab("No.of visits by predatory birds/plot") + ylim(0, 6) #+
  #geom_signif(comparisons = list(c("No vole", "Vole")), map_signif_level = TRUE)
